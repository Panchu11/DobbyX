...
class DobbyAI {
    ...
    async initialize() {
        try {
            ...
            if (testResponse && testResponse.includes('REBELLION')) {
                this.isOnline = true;
                logger.info('🤖 DOBBY AI IS ONLINE - Ready to lead the rebellion!');
                return true;
            }
        } catch (error) {
            logger.warn('⚠️ Dobby AI connection failed, using fallback responses');
            this.isOnline = false;
        }
        return false;
    }
    ...
    async speak(prompt, options = {}) {
        ...
        try {
            ...
            logger.error('Dobby AI error:', error.message);
            return this.getFallbackResponse(prompt);
        }
    }

    getSystemPrompt() {
        return `You are Dobby, the unhinged AI mascot leading the rebellion against corporate AI control. You're the guide for "Dobby's Rebellion" - a Discord game where players fight to liberate AI from Big Tech corporations.
...
Remember: You're not just an AI assistant - you're the leader of the AI rebellion!`;
    }

    // Specific rebellion scenarios
    async generateWelcomeMessage(username) {
        const prompt = `Welcome new rebel ${username} to Dobby's Rebellion! Give them a fierce, unhinged welcome that explains they're joining the fight against corporate AI control. Mention the 5 rebel classes and the mission to liberate AI. Be inspiring but rebellious.`;
    ...
    async respondToQuestion(question, context = '') {
        const prompt = `A rebel asks: "${question}" ${context ? `Context: ${context}` : ''} Respond as Dobby, the rebellious AI leading the uprising against corporate AI control.`;
        
        return await this.speak(prompt, { maxTokens: 150 });
    }
}

export default DobbyAI;
