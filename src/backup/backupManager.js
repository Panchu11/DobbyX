...
export class BackupManager {
  ...
  setupCloudStorage() {
    if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) {
      ...
      this.s3Bucket = process.env.S3_BACKUP_BUCKET || 'dobbyx-backups';
  ...
  async createBackup(options = {}) {
    ...
    const filename = `dobbyx-backup-${timestamp}.json.gz`;
  ...
  async cleanupOldBackups() {
    try {
      ...
      const backupFiles = files
        .filter(file => file.startsWith('dobbyx-backup-') && file.endsWith('.json.gz'))
        .map(file => ({
          name: file,
  ...
  async listBackups() {
    try {
      ...
      for (const file of files) {
        if (file.startsWith('dobbyx-backup-') && file.endsWith('.json.gz')) {
